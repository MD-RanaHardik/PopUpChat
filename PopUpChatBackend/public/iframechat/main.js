...
async function StartChatIo(id){
    ...
    await fetch(`${API_HOST}/client/getwidget/${id}`).then((res) => res.json()).then((data) => {
        if (data[0]["Property_status"]) {
            let colorcode = data[0]["Widget"]["Widget_color"];
            console.log(data[0]["Widget"]["Widget_color"]);
            document.body.innerHTML +=`
                <iframe src="https://popupchat.onrender.com/iframechat/chat.html?id=${id}" id="chat" frameborder="0" style="visibility: hidden !important; background: transparent !important; height: 480px !important; width: 310px !important;  z-index:2147483647 !important; position: fixed !important;bottom: 13% !important;right: 2% !important;"></iframe>
